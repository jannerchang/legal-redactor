...
`v0.1.1` 是一个适合作为阶段检查点的版本，而不是单纯的代码整理。它的主要价值在于：

- **安全性提升**：修复样本诊断页面里手工样本值未转义的 XSS 风险，并补上覆盖
  `delete`、`modify`、`add`、`keep` 和人工理由分支的回归测试。这是本轮最高优先级价值。
- **性能可扩展性提升**：行政区划扫描从逐词全文查找改为 trie 多模式扫描。随着全国行政区划库和长文书输入增长，
  这类优化比微调单个正则更有长期收益。
- **运行可靠性提升**：本地 LLM 调用增加重试退避，日志统一到项目 logger；模型偶发失败时更容易诊断，
  也减少单次本地 MLX 波动直接打断流程的概率。
- **识别质量修复**：保留 `李某`、`罗某`、`王某某` 这类匿名人名中的 `某`，避免清洗阶段把有效当事人名截短后误删。
- **可维护性提升**：把 detector filter、postprocess 链、pipeline 步骤、Web 模板渲染拆到独立模块，并保留兼容导出和专项测试。
  这类重构本身不直接提高识别率，但降低后续调整规则、定位回归和拆分运行时实验的成本。
- **基准决策能力提升**：新增 M8 运行时 benchmark 报告路径，用于比较 `mlx_lm.server`、Rapid-MLX 和纯规则兜底等候选；
  它只产生可复核指标，不会自动切换默认模型。
- **交付卫生提升**：移除已跟踪的 `.DS_Store`，加入忽略规则，并把版本元数据统一提升到 `0.1.1`。

需要注意的是，本轮包含较大的结构性重构。它的价值成立依赖于主线验证：PR #6 合并前后完整测试均通过
`319 passed`，发布元数据 PR #7 通过 MCP / Office API / Web 相关测试 `89 passed`。后续如果继续优化识别率，
应优先用 M6/M8 报告和真实样本回归衡量，不应仅凭重构后的代码形态判断效果。
