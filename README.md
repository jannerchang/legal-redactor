...
首页的“识别模式”默认保持“逐句窗口（稳定）”。可按单次处理切换到“整篇文书（实验）”：本地 LLM 一次读取单篇文书并返回案件级实体登记表，程序再校验原文字符串、定位全部精确 span，并沿用现有确定性映射与替换流程。实验模式单篇硬上限为 120000 字符，不截断；超限、无效登记表或模型调用失败时会明确回退。结果页只展示模式、逻辑模型 ID、状态、文档数、调用数、识别耗时、降级与冲突计数，不展示 prompt、response、实体登记表、证据或模型权重路径。
