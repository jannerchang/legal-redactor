...
class LegalEntityAuditor:
    ...
    def _build_sentence_extraction_prompt(self, windows: list[dict[str, str]], enable_samples: bool = True) -> str:
        ...
        return (
            ...
            f"{few_shot_part}"
            "边界判断要求：先在内部检查分词、候选实体、左右边界和实体类型，但最终不要输出分词或推理过程。\n"
            "- 长机构名优先：能组成完整公司/律所/银行/学校/委员会/工程局名称时，输出完整名称，不要只输出字号或后缀。\n"
            "- 括号关系保留：`原名称`、`曾用名`、`简称`、`以下简称`、`下称` 明确指向同一机构时，当前名、原名、简称都可作为机构实体。\n"
            "- 公司字号谨慎：字号单独出现且上下文未说明其为公司/机构时，不要输出；带“公司/集团/银行/律所”等语义时再输出。\n"
            "- 人名短片段谨慎：2 字人名如果只是更长人名、机构名、案号或项目名的一部分，不要输出。\n"
            "- 地址/项目边界：小区、花园、华府、澜庭、蓝庭、产业园、标段等作为项目或楼盘名时，保留完整名称。\n\n"
            "需要抽取的实体类型：\n"
