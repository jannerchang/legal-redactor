...
def _is_valid_company_variant(text: str) -> bool:
    ...
    if any(stripped.endswith(suffix) for suffix in _NAMED_BUSINESS_OUTLET_SUFFIXES):
        return len(stripped) >= 4
...
class LegalEntityAuditor:
    ...
    def _build_registry_repair_prompt(self, text: str) -> str:
        return (
            ...
            "前三个数组只列去重后的原文实体名称；organizations 必须覆盖全文所有有专名的公司和机构，不限于当事人；"
            "locations 必须覆盖原文出现的省、地级市和以‘区’结尾的市辖区名称。"
    ...
    def _build_full_document_registry_prompt(
        ...
        return (
            ...
            "只登记 person、organization、location；person 必须是具体自然人姓名。"
            "organization 必须登记全文出现的所有有专名的具体法律主体或经营主体，不限于当事人；"
            "逐段清点公司、集团、银行、分支行、律所、医院、学校、经营部、安装部、安装队、经销处等名称；"
            "非当事人、代发工资主体、供应商、承包商、付款主体和证据中出现的具体机构也必须登记，不得因诉讼角色或仅出现一次而遗漏。"
    ...
    def _build_full_document_supplement_prompt(
        ...
        return (
            ...
            "名称必须逐字来自原文。person 必须是具体自然人姓名。organization 必须登记全文出现的所有有专名的具体法律主体或经营主体，不限于当事人；"
            "先逐段复核第一轮清单，再重点清点公司、集团、银行、分支行、律所、医院、学校、经营部、安装部、安装队、经销处；"
            "非当事人、代发工资主体、供应商、承包商、付款主体和证据中出现的具体机构也必须补齐，不得因诉讼角色或仅出现一次而遗漏。"
