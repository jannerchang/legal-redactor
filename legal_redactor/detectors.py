...
_FALSE_PERSON_WORDS = frozenset({
    ...
    "鉴于", "依照", "按照", "鉴于", "予以", "不予", "予以",
    "共同", "各自", "应当", "可以", "不得", "已经", "尚未",
    "予以", "进行", "作出", "提出", "提交", "出具", "提供",
    ...
    for pattern in _FALLBACK_PERSON_PATTERNS:
        for match in pattern.finditer(text):
            ...
            if not re.fullmatch(r"[一-龥]{2,4}", value):
                continue
            # 姓不能是罕见单字指示词
            if value[0] in "被原申本上该此前述依根关由自因对从与和或及至向在已请告人我持合配旗交说证其们起于当":
                continue
...
def _is_false_org(value: str) -> bool:
    ...
    # 过滤带有明显合同、证据、诉讼等非公司机构特征的词（如“一审判决公司”、“慕尚公司签订的施工合同...证据佐证”）
...
def _looks_like_false_location(text: str, start: int, end: int, raw: str) -> bool:
    ...
    if raw_stripped in {"省", "市", "区", "县", "镇", "乡", "村", "街道", "社区", "自治区", "自治州", "市甲", "省丙"}:
        return True
