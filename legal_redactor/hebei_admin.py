...
class HebeiAdminDivisionDetector:
    source = "hebei_admin_db"

    def __init__(self, db_path: str | Path = "data/hebei_admin_divisions.sqlite") -> None:
            ...
            while True:
                ...
                candidates.append(
                    Candidate(
                        ...
                        risk_level="medium",
                        auto_redact=True,
                        reason=f"河北省行政区划/基层组织地名库：{term.canonical_name}",
                        metadata={
        ...
        try:
            ...
            with conn:
                for row in conn.execute(
                    ...
                    if _should_add_full_name_term(row["level"], row["name"], row["full_name"]):
                        self._add_term(
                            terms,
                            text=row["full_name"],
                    ...
                    for short_alias in _direct_admin_short_aliases(row["level"], row["name"]):
                        self._add_term(
                            terms,
                            text=short_alias,
                            canonical_name=row["full_name"] or row["name"],
...
def _should_add_full_name_term(level: str, name: str | None, full_name: str | None) -> bool:
    ...
    if value == (name or "").strip():
        return True
    return level in {"village", "community"}
